...
from drqa.layers import mt5_embedding, wc_embedding


def main():
    args, log = setup()
    log.info('[Program starts. Loading data...]')
    train, dev, dev_y, embedding, id2w, opt = load_data(vars(args))
    log.info(opt)
    log.info(f'[Data loaded. train:{len(train)} dev:{len(dev)}]')

    #Load Pretrain model
    if opt['baseModel'] == 'mt5':
        baseEmbed = mt5_embedding()
    elif opt['baseModel'] == 'wangchan':
        baseEmbed = wc_embedding()
    else :
        baseEmbed = id2w
    ...
    if args.resume:
        ...
        if args.reduce_lr:
            ...
            log.info(f"[learning rate reduced by {args.reduce_lr} , current learning {model.optimizer.param_groups[0]['lr']:.6f}]")
        batches = BatchGen(dev, batch_size=args.batch_size, evaluation=True, gpu=args.cuda)
        preds_txt = []
        preds_idx = []
        for i, batch in enumerate(batches):
            predictions = model.predict(batch)
            preds_txt.extend(predictions[0])
            preds_idx.extend(func_idx2w(predictions[1], baseEmbed, opt))
            log.debug('> evaluating [{}/{}]'.format(i, len(batches)))
        em, f1 = score(preds_idx, dev_y)
    ...
    for epoch in range(epoch_0, epoch_0 + args.epochs):
        log.warning(f'Epoch {epoch} Starting training')
        # train
        ...
        error_loss_nan = 0
        for i, batch in enumerate(batches):
            score_s, score_e = model.get_update(batch)
            if (score_s.isnan().any() | score_e.isnan().any()):
              error_batch +=1
            else :
              model.update(batch)
            if i % args.log_per_updates == 0:
                log.info(f'> epoch [{epoch:2}] updates[{model.updates:6}] train loss[{model.train_loss.value:.5f}] error[{error_batch}]')
        log.debug(f'training time {datetime.now()-start}\n')
        # eval
        batches = BatchGen(dev, batch_size=args.batch_size, evaluation=True, gpu=args.cuda)
        preds_txt = []
        preds_idx = []
        for i, batch in enumerate(batches):
            ...
            log.debug('> evaluating [{}/{}]'.format(i, len(batches)))
        em, f1 = score(preds_idx, dev_y)
        log.warning(f"dev EM: {em:.5f} F1: {f1:.5f}")
        if args.save_dawn_logs:
            time_diff = datetime.now() - dawn_start
            log.warning("dawn_entry: {}\t{}\t{}".format(epoch, f1/100.0, float(time_diff.total_seconds() / 3600.0)))
        if f1 > best_val_score:
            best_val_score = f1
            model_file = os.path.join(args.model_dir, f'best_model.pt')
            model.save(model_file, epoch, [em, f1, best_val_score])
            if not args.save_best_only :
                copyfile( model_file, os.path.join(args.model_dir, f'checkpoint_em{em:.5f}_f1{f1:.5f}_epoch_{epoch}.pt'))
            log.info('[new best model saved.]')
        if epoch == epoch_0 + args.epochs - 1:
           model_file = os.path.join(args.model_dir, f'checkpoint_last_epoch_{epoch}.pt')
           model.save(model_file, epoch, [em, f1, best_val_score])
           log.info(f'[Saved mdel in last epoch {epoch}.]')
...
def setup():
    ...
    parser.add_argument('--data_file', default='ThaiWikiQA/data.pkl',
                        help='path to preprocessed data file.')
    parser.add_argument('--meta_file', default='ThaiWikiQA/meta.pkl',
                        help='path to load vocab/tag/ent/embedding')
    parser.add_argument('--model_dir', default='models',
                        help='path to store saved models.')
    parser.add_argument('--save_best_only', action='store_true',
                        help='only save the best models.')
...
def load_data(opt):
    with open(opt['meta_file'], 'rb') as f:
        meta = pickle.load(f, encoding='utf8')
    embedding = torch.Tensor(meta['embedding'])
    opt['pretrained_words'] = True
    opt['baseModel'] = meta['baseModel']
    ...
    return train, dev, dev_y, embedding, meta['vocab'], opt
...
def func_idx2w(answers_idx, baseEmbed, opt):
    answers_txt = []
    if opt['baseModel'] in ['mt5', 'wangchan']:
        for elem in answers_idx:
            answers_txt.append(" ".join([ baseEmbed.token_decode(int(idx)) for idx in elem ]))
    else :
        for elem in answers_idx:
            answers_txt.append(" ".join([baseEmbed[int(idx)] for idx in elem if idx != 0]))
    return answers_txt
...
def _exact_match(pred, answers):
    if pred is None or answers is None:
        return False
    pred = ''.join(_normalize_answer(pred).split())
    for a in answers:
